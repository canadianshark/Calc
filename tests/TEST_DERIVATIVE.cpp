...
TEST_CASE("Derivative: Multiplication (product rule)", "[derivative][product]") {
    ...
    SECTION("Product of three factors") {
        CHECK(differentiate("x*y*z", "x", {"y", "z"}) == "((((1*y)+(x*0))*z)+((x*y)*0))");
    }

    SECTION("Product with functions") {
        CHECK(differentiate("x*sin(x)", "x") == "((1*sin(x))+(x*(cos(x)*1)))");
    }
}

// Тест 5: Производная частного
TEST_CASE("Derivative: Division (quotient rule)", "[derivative][division]") {
    ...
    SECTION("Complex quotients") {
        CHECK(differentiate("(x+1)/(x-1)", "x") == "((((1+0)*(x-1))-((x+1)*(1-0)))/((x-1)^2))");
    }
}

TEST_CASE("Derivative: Power function", "[derivative][power]") {
    SECTION("Power with constant exponent") {
        CHECK(differentiate("x^2", "x") == "((x^2)*((0*log(x))+((2*1)/x)))");
        CHECK(differentiate("x^3", "x") == "((x^3)*((0*log(x))+((3*1)/x)))");
        CHECK(differentiate("x^n", "x", {"n"}) == "((x^n)*((0*log(x))+((n*1)/x)))");
    }

    SECTION("Constant to variable power") {
        CHECK(differentiate("2^x", "x") == "((2^x)*((1*log(2))+((x*0)/2)))");
    }

    SECTION("Variable to variable power") {
        CHECK(differentiate("x^x", "x") == "((x^x)*((1*log(x))+((x*1)/x)))");
    }

    SECTION("Complex power expressions") {
        CHECK(differentiate("x^(2*x)", "x") == "((x^(2*x))*((((0*x)+(2*1))*log(x))+(((2*x)*1)/x)))");
    }
}
...
TEST_CASE("Derivative: Exponential and logarithmic functions", "[derivative][explog]") {
    SECTION("Exponential") {
        ...
        CHECK(differentiate("exp(x^2)", "x") == "(exp((x^2))*((x^2)*((0*log(x))+((2*1)/x))))");
    }

    SECTION("Natural logarithm (log)") {
        CHECK(differentiate("log(x)", "x") == "((1/x)*1)");
        CHECK(differentiate("log(3*x)", "x") == "((1/(3*x))*((0*x)+(3*1)))");
        CHECK(differentiate("log(x^2)", "x") == "((1/(x^2))*((x^2)*((0*log(x))+((2*1)/x))))");
    }
}
...
TEST_CASE("Derivative: Square root", "[derivative][sqrt]") {
    ...
    SECTION("Square root of expression") {
        ...
        CHECK(differentiate("sqrt(x^2+1)", "x") == "((1/(2*sqrt(((x^2)+1))))*(((x^2)*((0*log(x))+((2*1)/x)))+0))");
    }
}
...
TEST_CASE("Derivative: Chain rule with nested functions", "[derivative][chain]") {
    ...
    SECTION("Logarithm of function") {
        CHECK(differentiate("log(sin(x))", "x") == "((1/sin(x))*(cos(x)*1))");
        CHECK(differentiate("log(exp(x))", "x") == "((1/exp(x))*(exp(x)*1))");
    }

    SECTION("Deep nesting") {
        CHECK(differentiate("sin(log(x^2))", "x") == "(cos(log((x^2)))*((1/(x^2))*((x^2)*((0*log(x))+((2*1)/x)))))");
...
TEST_CASE("Derivative: Combined rules", "[derivative][combined]") {
    SECTION("Product and chain rule") {
        CHECK(differentiate("x*sin(x^2)", "x") == "((1*sin((x^2)))+(x*(cos((x^2))*((x^2)*((0*log(x))+((2*1)/x))))))");
    }

    SECTION("Quotient and chain rule") {
        CHECK(differentiate("sin(x)/cos(x)", "x") == "((((cos(x)*1)*cos(x))-(sin(x)*(-(sin(x))*1)))/(cos(x)^2))");
    }

    SECTION("Power and chain rule") {
        CHECK(differentiate("sin(x)^2", "x") == "((sin(x)^2)*((0*log(sin(x)))+((2*(cos(x)*1))/sin(x))))");
    }
}
...
TEST_CASE("Derivative: Special cases", "[derivative][special]") {
    ...
    SECTION("Derivative of one") {
        ...
        CHECK(differentiate("x^0", "x") == "((x^0)*((0*log(x))+((0*1)/x)))");
    }
...
TEST_CASE("Derivative: Practical examples", "[derivative][practical]") {
    SECTION("Polynomial") {
        CHECK(differentiate("x^2+3*x+5", "x") == "((((x^2)*((0*log(x))+((2*1)/x)))+((0*x)+(3*1)))+0)");
    }

    SECTION("Complex expression") {
        CHECK(differentiate("x*sin(x)+x^2*cos(x)", "x") == "(((1*sin(x))+(x*(cos(x)*1)))+((((x^2)*((0*log(x))+((2*1)/x)))*cos(x))+((x^2)*(-(sin(x))*1))))");
    }
}